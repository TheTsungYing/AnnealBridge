...
class DWaveQPUOptions(BaseModel):
    ...
    chain_strength: Quantity | None = _FINITE  # None → Ocean 預設 (uniform_torque_compensation)
...
class LeapHybridBQMOptions(BaseModel):
    ...
    time_limit_seconds: Quantity | None = _FINITE  # None → sampler 最小值
...
class FujitsuDAOptions(BaseModel):
    ...
    time_limit_seconds: Count | None = Field(default=None, ge=1, le=3600)
    num_run: Count | None = Field(default=None, ge=1, le=1024)
    num_group: Count | None = Field(default=None, ge=1, le=16)
    num_output_solution: Count | None = Field(default=None, ge=1, le=1024)
...
class SolverPreferences(BaseModel):
    ...
    penalty_multiplier: Quantity = Field(default=2.0, allow_inf_nan=False)
