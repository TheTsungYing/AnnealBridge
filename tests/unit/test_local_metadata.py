...
``exact`` and ``simulated_annealing`` have no vendor facts to relay, but a
run still happened here: the metadata names the backend, says the run did
not leave this machine, and carries the model type the service stamped on
it. What a vendor would fill in — timing, solver id, quota, embedding — is
empty or null, and stays that way.
...
LOCAL_BACKENDS = ["exact", "simulated_annealing"]
