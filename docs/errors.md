...
| `INVALID_SOLVER_PREFERENCE` | A solver preference is outside its allowed range. | Correct the value, or omit it to use the default. Every numeric preference must be finite and `> 0`; a retry count may also be `0`. |
