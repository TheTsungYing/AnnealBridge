...
| `SB_VARIABLE_LIMIT` | The compiled problem has more variables than the `simulated_bifurcation` backend's dense-matrix limit (`ANNEALBRIDGE_SB_MAX_VARIABLES`), which sizes the `N × N` coupling matrix it would have to allocate. | Reduce the problem size, or use `simulated_annealing` or `tabu`. |
